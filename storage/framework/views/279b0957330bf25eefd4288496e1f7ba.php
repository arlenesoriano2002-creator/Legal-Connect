...
<head>
    ...
    <link rel="stylesheet" href="<?php echo e(asset('css/edit-account.css')); ?>">
    <!-- Bootstrap CSS -->
...
<body>
    ...
    document.getElementById('passwordChangeForm').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch('<?php echo e(route("account.request.password.change")); ?>', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    document.getElementById('otpVerificationForm').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch('<?php echo e(route("account.verify.otp.password")); ?>', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
 ...
 async function resendOtp() {
    ...
    try {
        const response = await fetch('<?php echo e(route("account.resend.otp")); ?>', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json'
...
function messageBackToAllOffices() {
    ...
    <script>
        ...
        <?php if(session()->has('tab_session')): ?>
        const tabSession = <?php echo json_encode(session('tab_session'), 15, 512) ?>;
...
<?php /**PATH D:\xampp\htdocs\Legal connect final\LegalConnect\resources\views\welcome.blade.php ENDPATH**/ ?>
...
{
    private const SESSION_KEY = 'staff_account_password_reset';
    ...
    {
        $user = $this->getAuthenticatedStaff();

        return view('diffun_staff.forgotPassword', [
            'user' => $user,
            'mode' => 'email',
    ...
    public function sendCode(Request $request)
    {
        ...
        if (strcasecmp($validated['email'], $user->email) !== 0) {
            return back()
                ->withInput()
                ->withErrors([
                    'email' => 'The email address must match your current staff account email.',
                ]);
        }
        ...
        return redirect()
            ->route('staff.account.settings.forgot-password.verify')
    ...
    public function showVerifyForm()
    {
        ...
        if (!$this->hasValidStateForUser($user, $state)) {
            ...
            return redirect()
                ->route('staff.account.settings.forgot-password.email')
                ->withErrors([
    ...
    public function verifyCode(Request $request)
    {
        ...
        return redirect()
            ->route('staff.account.settings.forgot-password.reset')
    ...
    public function resetPassword(Request $request)
    {
        ...
        return redirect()
            ->route('staff.account.settings')
            ->with('success', 'Your password has been updated successfully!');
    }

    private function getAuthenticatedStaff(): User
    {
        $user = PerTabAuthHelper::getTabUser();

        if (!$user || $user->role !== 'diffun_staff') {
            ...
            Mail::mailer(config('mail.default'))->send(
                ...
                function ($message) use ($user, $fromAddress, $fromName) {
                    ...
                    $message->to($user->email)
                        ->subject('Staff Password Reset Code - Legal Connect');
                }
            );

            Log::info('Staff password reset code email sent successfully', [
                'user_id' => $user->id,
                'email' => $user->email,
            ]);

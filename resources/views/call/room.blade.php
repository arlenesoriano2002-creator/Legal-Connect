...
<head>
    ...
    <title>{{ $receiver->name }} - Video Call | Legal Connect</title>
...
<body>
    <div class="call-container">
        ...
        <div class="call-header">
            <div class="header-info">
                <div class="call-user-avatar">{{ substr($receiver->name, 0, 1) }}</div>
        ...
        <div class="video-container">
            ...
            <div class="video-wrapper remote-wrapper">
                ...
                <div class="no-video" id="remoteNoVideo">
                    ...
                    <div>Waiting for {{ $receiver->name }}...</div>
                </div>
            </div>
    ...
    <script>
        ...
        function initializePusher() {
            callState.pusher = new Pusher('{{ env('PUSHER_APP_KEY') }}', {
        ...
        async function initiateCall() {
            try {
                ...
                if (data.success) {
                    ...
                    updateCallStatus('Calling {{ $receiver->name }}...');
        ...
        async function acceptCall() {
            try {
                // Answer call
                const response = await fetch('{{ route('call.answer') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').content
        ...
        async function sendSdpOffer(offer) {
            try {
                await fetch('{{ route('call.sdp-offer') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').content
        ...
        async function sendSdpAnswer(answer) {
            try {
                await fetch('{{ route('call.sdp-answer') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').content
        ...
        async function sendIceCandidate(candidate) {
            try {
                await fetch('{{ route('call.ice-candidate') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').content

        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="content-modal">
                    ...
                    <form class="modal-form-grid" method="POST" action="{{ route('superadmin.lawyers.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="createLawyerOffice" name="law_office_id" required>
                                ...
                                @foreach ($lawOffices as $office)
                                    <option value="{{ $office->id }}" {{ old('law_office_id') == $office->id ? 'selected' : '' }}>
                                        {{ $office->law_office }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="content-modal">
                        ...
                        <form class="modal-form-grid" method="POST" action="{{ route('superadmin.lawyers.update', $lawyer) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div>
                                <label for="lawyerOffice{{ $lawyer->id }}">Law Office</label>
                                <select id="lawyerOffice{{ $lawyer->id }}" name="law_office_id" required>
                                    <option value="">Select a law office</option>
                                    @foreach ($lawOffices as $office)
                                        <option value="{{ $office->id }}" {{ ($lawyer->law_office_id ?? '') == $office->id ? 'selected' : '' }}>
                                            {{ $office->law_office }}
                                        </option>
                                    @endforeach
                                </select>
        ...
        <div class="modal fade" id="deleteLawyerModal{{ $lawyer->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="content-modal">
                        <p class="modal-description">Are you sure you want to delete {{ $lawyer->name ?? 'this lawyer' }}? This action cannot be undone.</p>
                        <div class="modal-footer px-0 pb-0">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <form method="POST" action="{{ route('superadmin.lawyers.delete', $lawyer) }}" style="display: inline;">
                                @csrf
                                @method('DELETE')

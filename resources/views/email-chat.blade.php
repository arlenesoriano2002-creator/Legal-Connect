...
<body>
                ...
                <div class="email-sidebar">
                    ...
                    <div id="conversations-section-content" class="section-content collapsed">
                        <div id="email-conversations-list">
                            @foreach($emailConversations as $conversation)
                            <div class="contact-item" onclick="selectEmail('{{ $conversation['sender_email'] }}', '{{ $conversation['sender_name'] ?? $conversation['sender_email'] }}')">
                                <div class="contact-name">{{ $conversation['sender_name'] ?? $conversation['sender_email'] }}</div>
                                <div class="contact-email">{{ $conversation['sender_email'] }}</div>
                                <div style="font-size: 0.8em; margin-top: 4px; opacity: 0.7;">
                                    {{ Str::limit($conversation['latest_subject'], 30) }}
                                </div>
                            </div>
                            @endforeach
                        </div>
    ...
    let currentEmail = '';
        let currentName = '';
        let currentUserEmail = '{{ Auth::check() ? Auth::user()->email : "" }}'; // FIXED: Check for null user
   ...
   function initializeComposeForm() {
    ...
    async function fetchNewEmails() {
        ...
        try {
            ...
            const response = await fetch('{{ route('email.inbox') }}', {
                method: 'GET',
                headers: {
    ...
    async function loadConversation(email) {
        ...
        try {
           const baseConversationUrl = `{{ url('/email/conversation') }}`;
...
function formatTimestampForDisplay(timestamp) {
    ...
    async function checkForNewMessages() {
        ...
        try {
            const baseCheckNewUrl = `{{ url('/email/conversation') }}`;
    ...
    async function pollLatestMessage() {
        if (!currentEmail) return;
        try {
            const latestUrl = `{{ url('/email/latest') }}/${encodeURIComponent(currentEmail)}?sync=0`;

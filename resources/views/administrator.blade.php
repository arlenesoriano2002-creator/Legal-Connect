                            ...
                            <div class="month-calendar">
                                ...
                                <div class="calendar-office-display" style="display: flex; align-items: center; gap: 12px; margin-bottom: 15px;">
                                    <span style="background: #0d6efd; color: white; padding: 6px 12px; border-radius: 4px; font-weight: 500; font-size: 13px;">
                                        📍 {{ Auth::user()->law_office_id && Auth::user()->lawOffice ? Auth::user()->lawOffice->law_office : 'Administrator - All Offices' }}
                                    </span>
                                    ...
                                    <select id="adminOfficeSelector" class="form-select form-select-sm" style="max-width: 200px;" onchange="setAdminOffice(this.value)">
                                        ...
                                        <option value="{{ $office->id }}" {{ session('law_office_id') == $office->id ? 'selected' : '' }}>
                                            {{ $office->law_office }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div id="cordonView" class="view-pane" style="display: none;">
                            <div class="month-calendar">
                                <div class="calendar-header">
                                    ...
                                    <h3 id="cordonCurrentMonthYear">{{ date('F Y') }}</h3>
...
<script>
    window.currentUser = {
        ...
        law_office_id: {{ Auth::user()->law_office_id ?? 'null' }},
        law_office: '{{ Auth::user()->law_office_id && Auth::user()->lawOffice ? Auth::user()->lawOffice->law_office : "Administrator - All Offices" }}'
    };
    ...
    @if(session('law_office_id'))
        sessionStorage.setItem('selectedOfficeId', '{{ session('law_office_id') }}');
    @endif
...
<script src="{{ asset('js/cordon-calendar.js') }}"></script>

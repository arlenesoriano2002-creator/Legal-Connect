<link rel="stylesheet" href="{{ asset('css/partials/backup-manager.css') }}">
    ...
    @forelse($backups as $backup)
        <div class="backup-card">
            ...
            <div class="backup-card-actions">
                <!-- NEW: View Button -->
                <button class="backup-btn view-btn" data-backup-id="{{ $backup->id }}">
                    <i class="fa-solid fa-eye"></i> View
                </button>

                <!-- FIX: Update download route to use backup ID instead of filename -->
                <a href="{{ route('backup.download', $backup->id) }}" class="backup-btn download-btn">
                    <i class="fa-solid fa-file-arrow-down"></i> Download
                </a>

                <!-- FIX: Update delete route to use backup ID instead of filename -->
                <form action="{{ route('admin.backup.delete.byid', $backup->id) }}"
                    method="POST"
                    class="delete-backup-form"
                    data-file="{{ $backup->decrypted_file_name ?? 'Backup' }}">
                    @csrf
                    @method('DELETE')

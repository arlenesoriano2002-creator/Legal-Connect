...
<body style="margin:0; padding:24px; background:#f5f7fb; font-family:Arial, sans-serif; color:#1f2937;">
    <div style="max-width:600px; margin:0 auto; background:#ffffff; border-radius:12px; padding:32px; border:1px solid #e5e7eb;">
        <h2 style="margin-top:0; color:#2f4050;">LegalConnect Staff Password Reset</h2>
        <p>Hello {{ $user->name }},</p>
        <p>We received a request to reset the password for your staff account.</p>
        ...
        <div style="margin:24px 0; padding:20px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:10px; text-align:center;">
            ...
            <div style="font-size:32px; font-weight:700; letter-spacing:8px; color:#14532d;">{{ $verificationCode }}</div>
        </div>

        <p>This code will expire on <strong>{{ $expiresAt->format('M d, Y h:i A') }}</strong>.</p>

...
<body>
        ...
        <div id="sidebar-wrapper">
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="#messagesSubmenu" 
                    class="list-group-item list-group-item-action {{ request()->is('email-chat') || request()->is('sms-chat') || request()->is('messages/*') ? 'active' : '' }}"
                    data-bs-toggle="collapse" 
                    aria-expanded="{{ request()->is('email-chat') || request()->is('sms-chat') || request()->is('messages/*') ? 'true' : 'false' }}">
                    <i class="fas fa-envelope"></i>
                    <span>Messages</span>
                    <i class="fas fa-chevron-down"></i>
                </a>
                <div class="submenu collapse {{ request()->is('email-chat') || request()->is('sms-chat') || request()->is('messages/*') ? 'show' : '' }} list-group" id="messagesSubmenu">
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-10 col-xl-8">
                        <div class="settings-wrapper">
                            ...
                            <!-- Profile Update Form -->
                            <form method="POST" action="{{ route('admin.account.settings.update.profile') }}" enctype="multipart/form-data" id="profileForm">
                                @csrf
                                @method('PUT')
                                        ...
                                        <div class="col-md-4 text-center">
                                            <div class="avatar-section">
                                                <div class="avatar mb-3" id="avatarPreview">
                                                    @if($user->image)
                                                        <img src="{{ asset('storage/' . $user->image) }}" alt="{{ $user->name }}" id="avatarImage" class="img-fluid rounded-circle">
                                                    @else
        ...
        <div class="modal fade" id="changePasswordModal" tabindex="-1" aria-labelledby="changePasswordModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{ route('admin.account.settings.update.password') }}" id="passwordForm">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            ...
                            <div class="settings-card">
                                <div class="form-group mb-3">
                                    ...
                                    <div class="mt-2">
                                        <a href="{{ route('admin.account.settings.forgot-password.email') }}" class="text-decoration-none small fw-semibold">
                                            Forgot Password?
                                        </a>
                                    </div>
                                </div>
    ...
    function resetForm() {
        ...
        const originalName = '{{ old("name", $user->name) }}';
        const originalEmail = '{{ old("email", $user->email) }}';
        const originalPhone = '{{ old("cp_number", $user->cp_number) }}';
        ...
        @if($user->image)
            if (avatarPreview) {
                avatarPreview.innerHTML = '<img src="{{ asset("storage/" . $user->image) }}" alt="{{ $user->name }}" id="avatarImage" class="img-fluid rounded-circle">';
            }
        @else
            if (avatarPreview) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    SessionHijackDetector.init({{ auth()->id() }});

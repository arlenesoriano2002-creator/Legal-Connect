...
<body>
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <img src="{{ asset($imagePath) }}" 
                                                            ...
                                                            onerror="this.onerror=null; this.src='{{ asset('staff_images/default-avatar.png') }}';">
                                                    </td>
                                                    <td>{{ $staff->name }}</td>
                                                    <td>{{ $staff->email }}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @else
                                                            <span class="badge bg-secondary">{{ ucfirst(str_replace('_', ' ', $staff->role ?? 'No role')) }}</span>
                                                        @endif
                                                    </td>
                                                    <td>{{ $staff->law_office ?? 'No Office Assigned' }}</td>
                                                    <td>{{ $staff->created_at->format('M d, Y') }}</td> <!-- This should be CREATED date -->
                                                    <td>
                                                        <button onclick='openEditStaffModal(@json($staff))' 
    ...
    <div id="createStaffModal" class="modal">
        <div class="modal-content">
            ...
            <form action="{{ route('adminAccount.staff.create') }}" method="POST" enctype="multipart/form-data" id="createStaffForm">
                @csrf
                ...
                <input type="hidden" name="law_office_id" value="{{ $user->law_office_id ?? '' }}">
                <div class="mb-3">
                    <label class="form-label">Law Office</label>
                    <div class="form-control-plaintext text-muted">
                        {{ $user->law_office ?? 'No Office Assigned' }}
                    </div>
    ...
    <div id="editStaffModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editStaffForm" method="POST" enctype="multipart/form-data" action="{{ route('adminAccount.staff.update', ['id' => '__ID__']) }}">
                @csrf
                @method('PUT')
    ...
    <script>
        ...
        function openEditStaffModal(staff) {
            ...
            form.action = '{{ route("adminAccount.staff.update", ["id" => "__ID__"]) }}'.replace('__ID__', staff.id);
        }
        ...
        function confirmDelete(staffId) {
            ...
            form.action = '{{ route("adminAccount.staff.delete", ["id" => "__ID__"]) }}'.replace('__ID__', staffId);
        }
    ...
    document.querySelectorAll('.staff-image').forEach(img => {
        img.onerror = function() {
            ...
            if (!currentSrc.includes('staff_images/')) {
                ...
                this.src = '{{ url("staff_images") }}/' + filename;
            } else {
                this.src = '{{ asset("staff_images/default-avatar.png") }}';
            }
        };
    });
    ...
    setTimeout(function() {
        document.querySelectorAll('.staff-image').forEach(img => {
            ...
            testImg.onerror = function() {
                ...
                img.src = '{{ asset("staff_images/default-avatar.png") }}';
            };
            testImg.src = img.src;
        });
    }, 1000);

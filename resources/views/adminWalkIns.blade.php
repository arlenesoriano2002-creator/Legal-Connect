...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.datatables.net/buttons/2.4.1/css/buttons.bootstrap5.min.css">
     ...
     <link rel="stylesheet" href="{{ asset('css/adminWalkInLogs.css') }}">
    <style>
        .dataTables_info {
                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                {{-- <div class="col-md-6">
                                    <div class="filter-group">
                                        <label class="form-label"><strong>Filter by Branch</strong></label>
                                        <div class="input-group">
                                            <span class="input-group-text">
                                            ...
                                            <select class="form-select" id="branchFilter">
                                                ...
                                                <option value="Diffun">Diffun Branch Office</option>
                                                <option value="Cordon">Cordon Branch Office</option>
                                            </select>
                                            <button class="btn btn-outline-secondary" type="button" id="clearBranchFilter" title="Clear branch filter">
        ...
        function init() {
            ...
            try {
                    ...
                    searchTimeout = setTimeout(function() {
                        ...
                        if (searchTerm) {
                            ...
                            table.rows().every(function() {
                                ...
                                {{-- var branch = rowNode.cells[4].textContent.toLowerCase(); --}}
                ...
                $('#branchFilter').on('change', function() {
                    var selectedBranch = (this.value || '').trim();
                    console.log('Branch filter changed to:', selectedBranch);
                    
                    var visibleRows = 0;
                    table.rows().every(function() {
                        var rowNode = this.node();
                        var branchCell = (rowNode.cells[4].textContent || '').trim();
                        
                        var match = !selectedBranch || branchCell === selectedBranch;
                        $(rowNode).toggle(match);
                ...
                $('#saveExcelBtn').on('click', function(e) {
                    ...
                    console.log('Exporting Excel with search:', searchVal{{-- , 'branch:', branchVal --}});
                    ...
                    $.ajax({
                        url: '{{ route("admin.walkins.export.excel") }}',
                        type: 'POST',
                ...
                $('#savePdfBtn').on('click', function(e) {
                    ...
                    console.log('Exporting PDF with search:', searchVal{{-- , 'branch:', branchVal --}});
                    console.log('CSRF Token:', csrfToken);
                    ...
                    $.ajax({
                        url: '{{ route("admin.walkins.export.pdf") }}',
                        type: 'POST',
                        headers: { 'X-CSRF-TOKEN': csrfToken },
                        data: { 
                            search: searchVal{{-- , 
                ...
                function loadBackupLogs() {
                    console.log('Loading backup logs from:', '{{ route("admin.walkins.backup.logs") }}');
                    $.get('{{ route("admin.walkins.backup.logs") }}', function(response) {
                ...
                function downloadBackupFile(backupId) {
                    window.location.href = '{{ route("admin.walkins.download.file", "") }}/' + backupId;
                }
                ...
                function previewBackupFile(backupId) {
                    ...
                    $.ajax({
                        url: '{{ route("admin.walkins.view.backup", "") }}/' + backupId,
                        type: 'GET',
                        dataType: 'json',
                        success: function(response) {
                            if (response.success) {
                                ...
                                // Set download link
                                $('#downloadPreviewBtn').attr('href', '{{ route("admin.walkins.download.file", "") }}/' + backupId);
                ...
                function deleteBackupFile(backupId, modalInstance) {
                    ...
                    $.ajax({
                        url: '{{ route("admin.walkins.delete.backup", "") }}/' + backupId,
                        type: 'DELETE',
